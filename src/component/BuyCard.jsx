...
const BuyCard = ({value , titles,  price, setvalue}) => {
 ...
 setshow(true)
  ...
  const amtprice = parseFloat(price.replace('$', ''));
  
  // Calculate the total amount with two decimal places
  const amt = (value * amtprice).toFixed(2);
  return (
    <>
  ...
  <div className="btnwallet">
    <button className="button" onClick={buy}>
      ...
      {(show ) && <div  className="orderplate">  
        <div className='Modal mg'>

          <img className='modalimg mg' src="src/pic/Eo_circle_green_checkmark.svg.png" alt="a" />
           <p className='mg'> {value} {titles}  has been ordered.<br></br> <br></br>  Your price is $ {amt}</p>
        <button onClick={close} className='closebut mg' > OK </button>
        </div>
